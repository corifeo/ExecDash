...
- **Initiative editor:** the projected value says what the category indicator measures and that it is optional. A loss reduction needs a linked risk: the slider and the value estimate appear only once a risk is linked, and a reduction left without links is cleared on save. While an item has unsaved changes its Done button is highlighted.
