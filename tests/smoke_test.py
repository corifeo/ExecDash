...
def main():
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_selector("#sec-4")
        page.click('#sec-4 .roisum button[data-act="view"][data-a="timeline"]')
        page.wait_for_selector("#tl .grow")
        rows = page.locator("#tl .grow").count()
        results.append(check("timeline lists every initiative", rows == page.locator("#l4body .init").count() or rows > 0))
        results.append(check("timeline shows a cumulative chart", page.locator("#tl .cumchart .cumline").count() == 1))
        for i in range(rows):
            page.locator("#tl .grow").nth(i).click()
        results.append(check("timeline rows expand to linked risks", page.locator("#tl .grow.open .grisk").count() > 0))
        page.click('button[data-act="tlrange"][data-a="1y"]')
        short = page.locator("#tl .gaxis i").count()
        page.click('button[data-act="tlrange"][data-a="all"]')
        results.append(check("timeline range shortens the axis", short < page.locator("#tl .gaxis i").count()))
        page.click('button[data-act="tlview"][data-a="compact"]')
        results.append(check("timeline compact hides row detail", page.locator("#tl .gdet:visible").count() == 0))
        page.click('button[data-act="tlview"][data-a="detail"]')
        page.click('#hdr button[data-act="view"][data-a="config"]')
        page.wait_for_selector('button[data-act="tab"][data-a="io"]')
        page.click('button[data-act="tab"][data-a="io"]')
